...
func Matches1(sub Sample) bool {
	for attr, value := range sub {
		matches := false
		switch attr {
		case "children":
			matches = value == 3
...
func main() {
	...
	for s.Scan() {
		...
		var sue, av, bv, cv int
		var an, bn, cn string

		_, err := fmt.Sscanf(line, "Sue %d: %s %d, %s %d, %s %d", &sue, &an, &av, &bn, &bv, &cn, &cv)
		if err != nil {
			panic(err)
		}
		an = strings.TrimSuffix(an, ":")
...
var lines = `Sue 1: goldfish: 6, trees: 9, akitas: 0
