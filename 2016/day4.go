...
func (xs FrequencyTable) Len() int      { return len(xs) }
func (xs FrequencyTable) Swap(i, j int) { xs[i], xs[j] = xs[j], xs[i] }
func (xs FrequencyTable) Less(i, j int) bool {
...
func (name Name) IsValid() bool {
	...
	var elems FrequencyTable
	for letter, count := range counts {
		elems = append(elems, LetterFrequency{letter, count})
	}

	sort.Sort(elems)
	for i, b := range name.Checksum {
		if elems[i].Letter != byte(b) {
			return false
		}
	}
...
func Parse(line string) (name Name) {
	p := 0
	for p < len(line) && !isNum(line[p]) {
		s := p
		for p < len(line) && line[p] != '-' {
			p++
		}
		name.Segments = append(name.Segments, line[s:p])
		p++
	}

	{
		...
		name.Sector, _ = strconv.Atoi(line[s:p])
		p++
	}
...
func main() {
	...
	fmt.Printf("%v\n", Parse(`not-a-real-room-404[oarel]`).IsValid())
	fmt.Printf("%v\n", Parse(`totally-real-room-200[decoy]`).IsValid())
