...
func main() {
	...
	sort.Slice(crossings, func(i, k int) bool {
		return crossings[i].ManhattanLength() < crossings[k].ManhattanLength()
	})

	fmt.Printf("found %v crossings\n", len(crossings))
...
func ParseWire(s string) (Wire, error) {
	...
	for _, s := range strings.Split(s, ",") {
		...
		if len(s) == 1 {
			return wire, fmt.Errorf("invalid segment length %q", s)
		}

		dir := Direction(s[0])
		if dir != Left && dir != Right && dir != Down && dir != Up {
			return wire, fmt.Errorf("unable to parse %q", s)
		}

		length, err := strconv.Atoi(s[1:])
		if err != nil {
			return wire, fmt.Errorf("unable to parse %q: %w", s, err)
		}
