...
func (grid *Grid) TurnOn(from, to Pos) {
...
func (grid *Grid) TurnOff(from, to Pos) {
	for y := from.y; y <= to.y; y++ {
		for x := from.x; x <= to.x; x++ {
			grid.Light[y][x]--
			if grid.Light[y][x] < 0 {
...
func (grid *Grid) Toggle(from, to Pos) {
	for y := from.y; y <= to.y; y++ {
		for x := from.x; x <= to.x; x++ {
			grid.Light[y][x] += 2
		}
...
func (grid *Grid) Do(line string) {
	var err error
	var from, to Pos
	if strings.HasPrefix(line, "turn on ") {
		_, err = fmt.Sscanf(line, "turn on %d,%d through %d,%d", &from.x, &from.y, &to.x, &to.y)
		grid.TurnOn(from, to)
	} else if strings.HasPrefix(line, "turn off ") {
		_, err = fmt.Sscanf(line, "turn off %d,%d through %d,%d", &from.x, &from.y, &to.x, &to.y)
		grid.TurnOff(from, to)
	} else if strings.HasPrefix(line, "toggle ") {
		_, err = fmt.Sscanf(line, "toggle %d,%d through %d,%d", &from.x, &from.y, &to.x, &to.y)
		grid.Toggle(from, to)
	} else {
...
func (grid *Grid) Count(from, to Pos) int {
	total := 0
	for y := from.y; y <= to.y; y++ {
		for x := from.x; x <= to.x; x++ {
			total += int(grid.Light[y][x])
		}
	}
	return total
}

func (grid *Grid) SaveAs(filename string) {
	m := image.NewGray(image.Rect(0, 0, W, H))

	from, to := Pos{0, 0}, Pos{W, H}
	for y := from.y; y < to.y; y++ {
		for x := from.x; x < to.x; x++ {
			x := grid.Light[y][x]
			if x > 255 {
			...
			m.SetGray(x, y, color.Gray{uint8(x)})
		}
	}
...
func main() {
	...
	grid.TurnOn(Pos{0, 0}, Pos{W - 1, H - 1})
	grid.SaveAs("day6_1.png")
	grid.TurnOff(Pos{0, 0}, Pos{W - 1, H - 1})
	grid.SaveAs("day6_2.png")
	for s.Scan() {
		grid.Do(s.Text())
	}
	fmt.Println(grid.Count(Pos{0, 0}, Pos{999, 999}))
	grid.SaveAs("day6_r.png")
}

var lines = `turn off 660,55 through 986,197
